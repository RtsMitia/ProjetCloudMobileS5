...
export const NOTIFICATION_MESSAGES = {
  SIGNALEMENT: {
    ...
    STATUS_CHANGED: {
      ...
      message: (oldStatus: string, newStatus: string) =>
        `Le statut est passé de "${oldStatus}" à "${newStatus}"`,
    },
  },
  PROBLEME: {
    CREATED: {
    ...
    STATUS_CHANGED: {
      ...
      message: (oldStatus: string, newStatus: string) =>
        `Le statut du problème est passé de "${oldStatus}" à "${newStatus}"`,
    },
  },
};
 ...
 */
export function validateNotificationOutbox(data: any): { valid: boolean; errors: string[] } {
  const errors: string[] = [];

  if (!data.type) errors.push('type is required');
  if (!data.entityId) errors.push('entityId is required');
  if (!data.action) errors.push('action is required');
  if (!data.userToken) errors.push('userToken is required');
